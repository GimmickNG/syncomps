package syncomps 
{
	 ...
	 * @author Gimmick
	 */
	public class HBox extends SynComponent 
	{
		...
		private function init():void
		{
			...
			cl_scrollPane.setStyle(ScrollPaneStyle.SCROLL_POLICY, ScrollPaneStyle.POLICY_HORIZONTAL)
		...
		override protected function drawGraphics(width:int, height:int, state:String):void
		{
			...
			var currX:Number = num_xPadding
			const ySpace:Number = num_yPadding * 2
			dp_items.forEach(function alignItems(item:DisplayObject, index:int, array:Array):void
			{
				item.height = height - ySpace
				item.y = num_yPadding
				item.x = currX
				
				currX += item.width + num_spacing
			}, this);
		}
